...
export default function Fournisseurs() {
  ...
  const deleteFournisseur = async (id) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/Fournisseurs/${id}`, {
        method: 'DELETE',
      });
      ...
      fetchFournisseurs(); // Refresh the fournisseurs list after deletion
    } catch (error) {
      console.log(error);
    }
  };
    ...
    <div>
      <nav className="navbar text-secondary justify-content-between">
        ...
        <button id="btn" className="btn">
          <Link to="/CreatF">Nouveau Fournisseur</Link>
        </button>
      </nav>
      <form>
              ...
              <tr key={F.id}>
                ...
                <td>
                  <button className='btn bg-success'>
                    <Link to={`/EditF/${F.id}`}>Update</Link>
                  </button>
                  <button className='btn bg-danger' onClick={() => deleteFournisseur(F.id)}>
                    Supprimer
                  </button>
                </td>
              </tr>
